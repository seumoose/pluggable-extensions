...
public class ApplicationLauncher {
	public static void main(String[] args) throws IOException {
		...
		Optional<IRunnableExtension> extensionAExtensionVariant1 = extensionRegistration
				.getRunnableExtension("ExtensionA", "Variant1");
		Optional<IRunnableExtension> extensionAExtensionVariant2 = extensionRegistration
				.getRunnableExtension("ExtensionA", "Variant2");

		extensionAExtensionVariant1.ifPresent(IRunnableExtension::run);
		extensionAExtensionVariant2.ifPresent(IRunnableExtension::run);

		Optional<IPredicateExtension<String>> emailValidator = extensionRegistration
				.getPredicateExtension("ExtensionB", "EmailValidator");
		Optional<IPredicateExtension<String>> urlValidator = extensionRegistration
				.getPredicateExtension("ExtensionB", "UrlValidator");

		emailValidator.ifPresent(
				(IPredicateExtension<String> extension) -> extension.test("user@example.com"));
		urlValidator.ifPresent((IPredicateExtension<String> extension) -> extension
				.test("https://example.com"));

		// {@link AbstractPredicateExtension} implementation protects against
		// ClassCastException errors
		Optional<IPredicateExtension<Integer>> invalidEmailValidator = extensionRegistration
				.getPredicateExtension("ExtensionB", "EmailValidator");

		invalidEmailValidator
				.ifPresent((IPredicateExtension<Integer> extension) -> extension.test(1));
